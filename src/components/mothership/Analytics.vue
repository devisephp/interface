  ...
  <div>
    <div id="devise-admin-content" class="dvs-relative dvs-full">
      <div v-if="site !== null" class="dvs-p-8">
        <h2 class="dvs-mb-8" :style="{color: theme.panel.color}">{{ site.name }} Analytics</h2>
        ...
        <template v-if="analytics !== null">
          ...
          <div class="dvs-flex dvs-justify-between dvs-text-center dvs-text-xs dvs-mb-8">
            <div class="dvs-rounded-sm dvs-p-4 dvs-w-1/5" :style="theme.actionButton">
              <h2
                class="dvs-font-bold"
                :style="{color: theme.actionButton.color}"
              >{{ analytics['usage-totals'].Users }}</h2>
              <strong class="dvs-uppercase">Users</strong>
            </div>
            <div class="dvs-rounded-sm dvs-p-4 dvs-w-1/5" :style="theme.actionButton">
              <h2
                :style="{color: theme.actionButton.color}"
              >{{ analytics['usage-totals'].Sessions }}</h2>
              <strong class="dvs-uppercase">Sessions</strong>
            </div>
            <div class="dvs-rounded-sm dvs-p-4 dvs-w-1/5" :style="theme.actionButton">
              <h2
                :style="{color: theme.actionButton.color}"
              >{{ analytics['usage-totals']['Page Views'] }}</h2>
              <strong class="dvs-uppercase">Page Views</strong>
            </div>
            <div class="dvs-rounded-sm dvs-p-4 dvs-w-1/5" :style="theme.actionButton">
              <h2
                :style="{color: theme.actionButton.color}"
              >{{ analytics['usage-totals']['Bounce Rate'] }}</h2>
          ...
          <div class="dvs-flex dvs-flex-col lg:dvs-flex-row dvs-mb-8">
            ...
            <div class="lg:dvs-w-1/2 lg:dvs-pr-10">
              <div class="dvs-mb-16">
                ...
                >How are site sessions trending?</h4>
            ...
            <div class="lg:dvs-w-1/2 dvs-p-8 dvs-rounded dvs-shadow-lg" :style="theme.panelCard">
              <div class="dvs-mb-8">
                <h4
                  class="dvs-mb-4"
                  :style="{color: theme.panel.color}"
                >What are the top countries by sessions?</h4>
                <simple-table
                  class="dvs-w-full"
...
export default {
  ...
  methods: {
    ...mapActions('devise', ['getSiteAnalytics']),
    setDefaultAnalytics() {
    ...
    retrieveAnalytics() {
      ...
      if (this.mothershipApiKey) {
        ...
        this.getSiteAnalytics({ site: this.site.id, dates: this.analyticsDateRange }).then(
          response => {
            response.data.sessions.datasets.map((dataset, index) =>
              self.formatColors(dataset, index)
            );

            response.data.channels.datasets.map((dataset, index) =>
              self.formatColors(dataset, index)
            );

            response.data.browser.datasets.map((dataset, index) =>
              self.formatColors(dataset, index)
            );
            ...
            // })
            for (const date in response.data['date-searched']) {
              if (response.data['date-searched'].hasOwnProperty(date)) {
                response.data['date-searched'][this.formatDate(date)] =
                  response.data['date-searched'][date];
  ...
  computed: {
    ...mapGetters('devise', ['mothership', 'mothershipApiKey', 'sites']),
    options() {
      return {
        responsive: true,
